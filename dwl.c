...
Buffer *bufmon(Monitor *m) {
    size_t i;
    Buffer *buf = NULL;
    static Buffer *shared_pool[2] = {NULL}; // new pool for all
    for (i = 0; i < LENGTH(shared_pool); i++) {
        if (shared_pool[i] && !shared_pool[i]->busy) {
            buf = shared_pool[i];
            break;
        }
        if (!shared_pool[i]) {
